...
class Test_horriblesubsInfo(unittest.TestCase):
    ...
    def setUp(self):
        with open("/tmp/homepage.html", "w") as file_tmp:
            file_tmp.write('<html><body><div class="latest">'
                           '<div id="shokugeki-11">'
                           '<div class="resolutions-block">'
                           '<div class="linkful resolution-block">'
                           '<span id="720p"><span class="ind-link">'
                           '<a href="http://success/filefactory"></a></span>'
    ...
    def test_get_result_list(self, get_activated_hosters):
        plugin = autodl.plugins.horriblesubsInfo.HorriblesubsInfo([{
            ...
            'site': 'horriblesubsInfo', 'title': 'shokugeki',
            'episode': '11'}], url="/tmp/homepage.html")
        autodl.utils.get_activated_hosters.return_value = ['Filefactory',
                                                           '1fichier']
