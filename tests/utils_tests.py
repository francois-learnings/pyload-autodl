...
class TestGetActivatedHosters(unittest.TestCase):    
    ...
    def setUp(self):
        with open("/tmp/config.json", "w") as file_tmp:
            json.dump({"hosters":["uplea", "1fichier", "Filefactory", "tuslfile"], "supported_sites":{"animes":["mangaFrCom", "animeserv"], "series":["scenesources", "telechargementz"]}, "target_titles":{"animes":{"naruto": "None", "shoukugeki": "9"}, "series":{"atlantis": "10", "olympus": "None"}},"target_ep":{"shoukugeki": "9","atlantis": "10"}}, file_tmp)
    ...
    def test_get_activated_hosters(self):
        ...
        hosters = autodl.utils.get_activated_hosters(config_file="/tmp/config.json")
        #print hosters

        assert isinstance(hosters, list)
        assert_equal(hosters, [u'uplea', u'1fichier', u'Filefactory', u'tuslfile'])
